...
public class Fish2_Right : Fish2Move {
    ...
    private IEnumerator Fire()
    {
        ...
        {
            fishATK.Play();
            Vector3 p = new Vector2(0.001f, 0);
            for (int j = 0; j < 8; j++)
            {

                if (j == 0)
                {
                    bullet = GameObject.Instantiate(m_IceBullet, Fish2.transform.position+p, Quaternion.identity) as GameObject;
